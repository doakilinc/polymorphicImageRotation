 ...
 *         structure
 * Expects
 * - row, col, elementSize to be greater than or equal to 0, throws a CRE 
 * otherwise
 * - that it returns a non-null T2 struct
 * Note: The user should call UArray2_free to avoid valgrind after calling 
 * this function
************************/
 ...
 * Expects
 * - that array is non-null, throws a CRE otherwise
************************/
 ...
 * Return: none
 * Expects
 * - non-null T2 array, throws a CRE otherwise
************************/
void UArray2_map_row_major(T2 array, 
        void apply(int col, int row,  T2 array, void *elem, void *clPtr), 
        void *cl) 
{
        assert(array != NULL);
        /* creating a nested loop such that column indices vary more rapidly */
        for (int iRow = 0; iRow < array->rows; iRow++) {
                for (int jCol = 0; jCol < array->cols; jCol++) {
                        apply(jCol, iRow, array, UArray2_at(array, jCol, iRow),
                             cl);
                }
        }
}
 ...
 * - non-null T2 array, throws a CRE otherwise
************************/
void UArray2_map_col_major(T2 array, 
        void apply(int col, int row, T2 array, void *elem, void *clPtr), 
        void *cl) 
{
        assert(array != NULL);
